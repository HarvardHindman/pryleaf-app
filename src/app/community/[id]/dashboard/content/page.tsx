...
export default function ContentManagementPage() {
  ...
  const filteredContent = filter === 'all' 
    ? content 
    : content.filter(item => {
        if (filter === 'post') return item.content_type === 'article';
        if (filter === 'video') return item.content_type === 'video';
        if (filter === 'image') return item.content_type === 'resource';
        return true;
      });

  const stats = {
    total: content.length,
    posts: content.filter(c => c.content_type === 'article').length,
    videos: content.filter(c => c.content_type === 'video').length,
    images: content.filter(c => c.content_type === 'resource').length,
...
function ContentCard({ content, onEdit, onDelete }: any) {
  ...
  const mediaPreview = content.thumbnail_url || content.content_url;
        ...
        {content.minimum_tier_level > 0 && (
          <div className="absolute top-2 right-2">
            ...
            >
              ...
              Tier {content.minimum_tier_level}+
            </span>
          </div>
        )}
      </div>
...
function ContentUploadModal({ type, communityId, onClose, onSave }: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const contentType = type === 'video' ? 'video' : 'article';
      ...
      const payload = {
        ...
        thumbnail_url: contentType === 'video' ? mediaUrl || null : null,
        minimum_tier_level: formData.required_tier_level,
        is_published: true,
        scheduled_publish_at: formData.schedule_at || null
      };
