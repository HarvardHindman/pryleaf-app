...
import { BulkQuoteResult, RefreshStats } from '../types';
import { API_LIMITS } from '../constants';

export class BulkQuoteCacheService {
  private supabase;
  private apiKey: string;
  private maxSymbolsPerBatch = API_LIMITS.BULK_QUOTE_BATCH_SIZE; // Alpha Vantage limit
      ...
      for (let i = 0; i < batches.length; i++) {
        ...
        try {
          ...
          if (i < batches.length - 1) {
            ...
            await new Promise(resolve => setTimeout(resolve, API_LIMITS.RATE_LIMIT_DELAY));
          }
